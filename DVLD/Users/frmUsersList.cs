...
namespace DVLD.Users
{
    public partial class frmUsersList : Form
    {
        ...
        {
            if (tbUserFilterationValue.Visible == true && tbUserFilterationValue.Text == string.Empty)
            {
            ...
            else
            {
                ...
                {
                    dvUsersList.RowFilter = $"{ColumnName} = '{Convert.ToInt32(Value)}'";
                }

                else if (cbFilters.SelectedItem.ToString() == "IsActive")
                {
                    if (Value == string.Empty) dvUsersList.RowFilter = "";

                    else dvUsersList.RowFilter = $"{ColumnName} = {Convert.ToByte(Value)}";
                }
                else
                {
                    dvUsersList.RowFilter = $"{ColumnName} LIKE '{Value}%'";
                }
        ...
        {
            _FilterUsersListRows(cbFilters.SelectedItem.ToString().Trim(), tbUserFilterationValue.Text);
            _ResetRecords(this, EventArgs.Empty);
        }
        ...
        //{
        //    frmChangePassword frm = new frmChangePassword(Convert.ToInt32(dvgUsersList.CurrentRow.Cells["UserID"].Value));
        //    frm.ShowDialog();
        //}
